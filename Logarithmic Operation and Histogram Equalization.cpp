...
/*
	Function: Logarithmic Operation
	Input: Row and column values, original two-dimensional array matrix
	Return: Array matrix after logarithmic operation
*/
UNCHAR **logarithmic(UNCHAR **original,int row,int col){
	int i,j;
	int max;
	UNCHAR **logarith=(UNCHAR**)malloc(sizeof(UNCHAR*)*row);
	for(i=0;i<row;i++){
		logarith[i]=(UNCHAR*)malloc(sizeof(UNCHAR)*col);
	}
	for(i=0;i<row;i++){
		for(j=0;j<col;j++){
			if(original[i][j]>max){
	...
	for(i=0;i<row;i++){
		for(j=0;j<col;j++){
			logarith[i][j] = log( original[i][j] + 1 ) * 255 / log( max + 1 );
		}
	}
	return logarith;
}
...
/*
	...
	Input: Row and column values, original two-dimensional array matrix
	Return: Array matrix after histogram equalization
*/
UNCHAR **histogram(UNCHAR **original,int row,int col){
	int i,j;
	int num,rowcol;
	int level[256];
	double count[256]={0};
	double proportion[256]={0};
	rowcol = row * col;
	UNCHAR**equalization=(UNCHAR**)malloc(sizeof(UNCHAR*)*row);
	for(i=0;i<row;i++){
		equalization[i]=(UNCHAR*)malloc(sizeof(UNCHAR)*col);
	}
	for(i=0;i<row;i++){
		for(j=0;j<col;j++){
			num=original[i][j];
	...
	for(i=0;i<256;i++){
		count[i]=count[i]/rowcol;
		if(i==0)
			proportion[i]=count[i];
		else if(i>0)
			proportion[i]=proportion[i-1]+count[i];	
	}
	for(i=0;i<256;i++){
		level[i]=(int)(proportion[i] * 255 + 0.5);
	}
		
	for(i=0;i<row;i++){
		for(j=0;j<col;j++){
			num=original[i][j];
			equalization[i][j] = level[num];
		}
	} 
	return equalization;
}
...
int main(){
	...
	newvalue=logarithmic(newvalueY,height,width);

	fplogarith=fopen("logarithlena.bmp","wb");
	if(!fplogarith){
		printf("Fail to open!");
	}
	else{
		fwrite(&bmpfileheader,sizeof(FileHeader),1,fplogarith);
		fwrite(&bmpinfoheader,sizeof(InfoHeader),1,fplogarith);
		
		for(row=0;row<height;row++){
			for(col=0;col<width;col++){
				fwrite(*(newvalue+row)+col,sizeof(UNCHAR),1,fplogarith);
				fwrite(*(newvalue+row)+col,sizeof(UNCHAR),1,fplogarith);
				fwrite(*(newvalue+row)+col,sizeof(UNCHAR),1,fplogarith);
			}
		}
	...
	UNCHAR**newvaluee=(UNCHAR**)malloc(sizeof(UNCHAR*)*height);
	for(row=0;row<height;row++){
		newvaluee[row]=(UNCHAR*)malloc(sizeof(UNCHAR)*width);
	}
	
	newvaluee=histogram(newvalue,height,width);
	
	fphistogram=fopen("histogramlena.bmp","wb");
	if(!fphistogram){
		printf("Fail to open!");
	}
	else{
		fwrite(&bmpfileheader,sizeof(FileHeader),1,fphistogram);
		fwrite(&bmpinfoheader,sizeof(InfoHeader),1,fphistogram);
		
		for(row=0;row<height;row++){
			for(col=0;col<width;col++){
				fwrite(*(newvaluee+row)+col,sizeof(UNCHAR),1,fphistogram);
				fwrite(*(newvaluee+row)+col,sizeof(UNCHAR),1,fphistogram);
				fwrite(*(newvaluee+row)+col,sizeof(UNCHAR),1,fphistogram);
			}
		}
	...
	newrgbpointer->r=histogram(rgbpointer->r,height,width);
	newrgbpointer->g=histogram(rgbpointer->g,height,width);
	newrgbpointer->b=histogram(rgbpointer->b,height,width); 
